...
import styles from "./LikedActivitiesCss";
...
export default function LikedActivities({ navigation }) {
  const { data } = useContext(DataContext);
  const [all_coaches, setAll_coaches] = useState([]);
  const [loading, setLoading] = useState(true);
  useEffect(() => {
    axios
      .post(data.url + "/coach/liked-activities", { token: data.authToken })
      .then((res) => {
        ...
        <ScrollView style={styles.main_scroll_view}>
          ...
          {all_coaches.length == 0 ? (
            <Text style={styles.no_coach}>
              You havn't liked any activity yet
            </Text>
          ) : null}
        </ScrollView>
      )}
