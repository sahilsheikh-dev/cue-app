...
import styles from "./AwarenessGuidelineCss";
...
export default function AwarenessGuideline({ navigation, route }) {
  ...
  useEffect(() => {
    axios
      .post(data.url + "/coach/get-awareness-guidelines", {
  ...
  const confirm = () => {
    ...
    axios
      .post(data.url + "/coach/user-has-read-awareness-guideline", {
        token: data.authToken,
      })
      .then((res) => {
        if (res.data.alert != undefined) {
          ...
          navigation.dispatch(
            CommonActions.reset({
              index: 1,
              routes: [{ name: "Home" }, { name: "AwarenessMainOptions" }],
            })
          );
        }
      });
