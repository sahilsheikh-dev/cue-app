...
import styles from "./ConnectionGuidelineCss";
...
export default function ConnectionGuideline({ navigation, route }) {
  ...
  useEffect(() => {
    axios
      .post(data.url + "/coach/get-connection-guidelines", {
  ...
  const confirm = () => {
    ...
    axios
      .post(data.url + "/coach/user-has-read-connection-guideline", {
        token: data.authToken,
      })
      .then((res) => {
        if (res.data.alert != undefined) {
          ...
          navigation.dispatch(
            CommonActions.reset({
              index: 1,
              routes: [{ name: "Home" }, { name: "ConnectionMainOptions" }],
            })
          );
        }
      });
