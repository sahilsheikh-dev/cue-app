...
export default function AdOtp({ navigation }) {
  ...
  const tryOtp = () => {
    setLoading(true);
    if (enu(inp1, inp2, inp3, inp4, inp5)) {
      axios
        .post(data.url + "/ad/auth/otp", {
        ...
        .then((res) => {
          if (res.data.alert != undefined) {
            ...
            partial_login_together(res.data.token, "ad");
            // login("ad");
            // navigation.navigate("Coach-build-profile");
          }
        });
    } else {
  ...
  const resend_otp = () => {
    axios
      .post(data.url + "/ad/auth/resend-otp", {
        otpId: otpId,
      })
      .then((res) => {
        if (res.data.alert != undefined) {
