...
export default function ShopOtp({ navigation }) {
  const { data, partial_login, login } = useContext(DataContext);
  ...
  const tryOtp = () => {
    ...
    if (enu(inp1, inp2, inp3, inp4, inp5)) {
      axios
        .post(data.url + "/product/auth/otp", {
  ...
  const resend_otp = () => {
    axios
      .post(data.url + "/product/auth/resend-otp", {
        otpId: otpId,
      })
      .then((res) => {
        if (res.data.alert != undefined) {
